...
def save_wav(wav, path):
    ...
    scipy.io.wavfile.write(path, config.sample_rate, wav.astype(np.int16))


# def preemphasis(x):
#     return scipy.signal.lfilter([1, -config.preemphasis], [1], x)
#
#
# def inv_preemphasis(x):
#     return scipy.signal.lfilter([1], [1, -config.preemphasis], x)
...
def inv_spectrogram(spectrogram):
    ...
    return _griffin_lim(S ** config.power)  # Reconstruct phase
...
def trim_silence(wav):
    ...
    unused_trimed, index = librosa.effects.trim(
        ...
        frame_length=config.trim_fft_size,
        hop_length=config.trim_hop_size)

    num_sil_samples = int(
        config.num_silent_frames * config.hop_size)
